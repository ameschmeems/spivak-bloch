...
\subsubsection*{5. Let $a, b \in \N$. Prove that if $a + a = b + b$, then $a = b$.}

\begin{proof}
	Let $a,b \in \N$ such that $a + a = b + b$. Then $a + a = 2a$ and $b + b = 2b$, so $2a = 2b$, so $a = b$.
\end{proof}

\subsubsection*{6. Let $b \in \N$. Prove that
	\[\left\{n \in \N|1 \le n \le b\right\} \cup \left\{n \in \N | b + 1 \le n\right\} = \N\]
	\[\left\{n \in \N | 1 \le n \le b\right\} \cap \left\{n \in \N | b+1 \le n\right\} = \emptyset\]
}

\begin{proof}
	Let $A = \left\{n \in \N|1 \le n \le b\right\} \cup \left\{n \in \N | b + 1 \le n\right\}$. Then $A \subseteq \N$ and $1 \in A$. Now suppose that $a \in A$. We consider three cases:

	\begin{enumerate}
		\item[Case 1:] $1 \le a < b$
		
		Then $a + 1 \le b$, so $a + 1 \in A$.

		\item[Case 2:] $a = b$
		
		Then $a + 1 = b + 1$, so $a + 1 \in A$.

		\item[Case 3:] $a > b$
		
		Then $b + 1 \le a$, so $b + 1 < a + 1$, so $a + 1 \in A$.
	\end{enumerate}
	This means that if $a \in A$, then $a + 1 \in A$, so $A = \N$.
\end{proof}

\begin{proof}
	Let $A = \left\{n \in \N | 1 \le n \le b\right\} \cap \left\{n \in \N | b+1 \le n\right\}$ and $a \in \left\{n \in \N | 1 \le n \le b\right\}$. Then $a \le b$, so $a < b + 1$. It then follows that $a \notin \left\{n \in \N | b+1 \le n\right\}$, so $A = \emptyset$.
\end{proof}

\subsubsection*{7. Let $A \subseteq \N$ be a set. The set $A$ is closed if $a \in A$ implies $a + 1 \in A$. Suppose that $A$ is closed.}

	\begin{enumerate}
		\item[(1)] Prove that if $a \in A$ and $n \in N$, then $a + n \in A$.
		
		\begin{proof}
			Let $B = \left\{n \in \N | a + n \in A\right\}$. Then $1 \in B$. Now let $b \in B$. Then since $a + b \in A$, it follows that $a + (b + 1) = (a + b) + 1 \in A$, and $(b + 1) \in B$, so $B = \N$.
		\end{proof}
		
		\item[(2)] Prove that if $a \in A$, then $\left\{x \in \N | x \ge a\right\} \subseteq A$. 
		
		\begin{proof}
			Let $x \in \N$ such that $x \ge a$. We consider two cases:

			\begin{enumerate}
				\item[Case 1:] $x = a$
				
				Then trivially, $x \in A$.

				\item[Case 2:] $x > a$.
				
				Then there exists such $n \in \N$, that $a + n = x$. It then follows from part (1) that $x \in A$.
			\end{enumerate}
		\end{proof}
	\end{enumerate}

\subsubsection*{8. Suppose that the set $\N$ together with the element $1 \in \N$ and the function $s: \N \mapsto \N$, and that the set $\N'$ together with the element $1' \in \N'$ and the function $s':\N'\mapsto\N'$, both satisfy the Peano Postulates. Prove that there is a bijective function $f: \N \mapsto \N'$ such that $f(1) = 1'$ and $f\circ s = s'\circ f$.}

\begin{proof}
	The existence of a function $f: \N \mapsto \N'$ such that $f(1) = 1'$ and $f\circ s = s'\circ f$ follows from Theorem 1.2.4, so it remains to prove that $f$ is bijective.

	Let $g: \N' \mapsto \N$ be a function such that $g(1') = 1$ and $g \circ s' = s \circ g$. The existence of such a function follows from Theorem 1.2.4. Let $A = \left\{n \in \N: g \circ f(n) = n \right\}$. Then $g(f(1)) = g(1') = 1$, so $1 \in A$. Now let $a \in A$. Then \[ g(f(s(a))) = g(s'(f(a))) = s(g(f(a))) = s(a), \] so $s(a) \in A$, and therefore $A = \N$.

	Now let $B = \left\{n' \in \N': f \circ g(n') = n'\right\}$. Then $f(g(1')) = f(1) = 1'$, so $1 \in B$. Let $b \in B$. Then \[ f(g(s'(b))) = f(s(g(b))) = s'(f(g(b))) = s'(b), \] so $s'(b) \in B$, and therefore $B = \N'$.
\end{proof}

\section{Constructing the Integers}

\begin{definition}
	The relation $\sim$ on $\N \times \N$ is defined by $(a,b)\sim(c,d)$ if and only if $a+d = b+c$ for all $(a,b),(c,d)\in \N \times \N$.
\end{definition}
...
\begin{definition}
	The set of integers, denoted $\Z$, is the set of equivalence classes of $\N \times \N$ with respect to the equivalence relation $\sim$.

	The elements $\hat{0}, \hat{1} \in \Z$ are defined by $\hat{0} = [(1, 1)]$ and $\hat{1} = [(1 + 1, 1)]$. The binary operations $+$ and $\cdot$ on $\Z$ are defined by
	\[ [(a,b)] + [(c,d)] = [(a + c, b + d)] \]
	\[ [(a,b)] \cdot [(c,d)] = [(ac + bd, ad + bc)] \]
	for all $[(a,b)], [(c,d)] \in \Z$. The unary operation $-$ on $\Z$ is defined by $-[(a,b)] = [(b,a)]$ for all $[(a,b)] \in \Z$. The relation $<$ on $\Z$ is defined by $[(a,b)] < [(c,d)]$ if and only if $a + d < b + c$, for all $[(a,b)],[(c,d)] \in \Z$. The relation $\le$ on $\Z$ is defined by $[(a,b)] \le [(c,d)]$ if and only if $[(a,b)] < [(c,d)]$ or $[(a,b)] = [(c,d)]$, for all $[(a,b)], [(c,d)] \in \Z$.
\end{definition}

\begin{lemma}
	The binary operations $+$ and $\cdot$, and the unary operation $-$ and the relation $<$, all on $\Z$, are well-defined.
\end{lemma}
...
\begin{theorem}
	...
	\begin{enumerate}
		...
		\item $x + \hat{0} = x$ (Identity Law for Addition)
		\item $x + (-x) = \hat{0}$ (Inverses Law for Addition)
		\item $(xy)z = x(yz)$ (Associative Law for Multiplication)
		\item $xy = yx$ (Commutative Law for Multiplication)
		\item $x \cdot \hat{1} = x$ (Identity Law for Multiplication)
		\item $x(y + z) = xy + xz$ (Distributive Law)
		\item If $xy = \hat{0}$, then $x = \hat{0}$ or $y = \hat{0}$ (No Zero Divisors Law)
		\item Precisely one of $x < y$ or $x = y$ or $x > y$ holds (Trichotomy Law)
		\item If $x < y$ and $y < z$, then $x < z$ (Transitive Law)
		\item If $x < y$ then $x + z < y + z$ (Addition Law for Order)
		\item If $x < y$ and $z > \hat{0}$ then $xz < yz$ (Multiplication Law for Order)
		\item $\hat{0} \ne \hat{1}$ (Non-Triviality)
	\end{enumerate}
\end{theorem}

\begin{definition}
	Let $x \in \Z$. The number $x$ is positive if $x > \hat{0}$, and the number $x$ is negative if $x < \hat{0}$.
\end{definition}

\begin{theorem}
	Let $i: \N \mapsto \Z$ be defined by $i(n) = [(n + 1, 1)]$ for all $n \in \N$.
	\begin{enumerate}
		\item The function $i: \N \mapsto \Z$ is injective.
		\item $i(\N) = x \left\{ x \in \Z | x > \hat{0} \right\}$
		\item $i(1) = \hat{1}$
		\item Let $a,b \in \N$. Then 
			\begin{enumerate}
				\item[a.] $i(a + b) = i(a) + i(b)$.
				\item[b.] $i(ab) = i(a)i(b)$
				\item[c.] $a < b$ if and only if $i(a) < i(b)$.
			\end{enumerate}
	\end{enumerate}
\end{theorem}
...
\begin{lemma}
	Let $x,y,z \in \Z$.
	\begin{enumerate}
		\item If $x + z = y + z$ then $x = y$ (Cancellation Law for Addition)
		\item $-(-x) = x$
		\item $-(x + y) = (-x) + (-y)$
		\item $x \cdot 0 = 0$
		\item If $z \ne 0$ and if $xz = yz$ then $x = y$ (Cancellation Law for Multiplication)
		\item $(-x)y = -xy = x(-y)$
		\item $xy = 1$ if and only if $x = 1 = y$ or $x = -1 = y$
		\item $x > 0$ if and only if $-x < 0$, and $x < 0$ if and only if $-x > 0$
		\item $0 < 1$
		\item If $x \le y$ and $y \le x$ then $x = y$
		\item If $x > 0$ and $y > 0$, then $xy > 0$. If $x > 0$ and $y < 0$ then $xy < 0$
	\end{enumerate}
\end{lemma}

\begin{theorem}
	Let $x \in \Z$. Then there is no $y \in \Z$ such that $x < y < x + 1$.
\end{theorem}

\subsection*{Exercises}

\subsubsection*{2. Complete the proof of Lemma 1.3.2. That is, prove that the relation $\sim$ is transitive.}

\begin{proof}
	Let $(a,b), (c,d), (e,f) \in \N \times \N$ such that $(a,b) \sim (c,d)$ and $(c,d) \sim (e,f)$, that is $a + d = b + c$ and $c + f = d + e$. Then
	\[ a + d + c + f = b + c + d + e \]
	\[ (a + f) + (d + c) = (b + e) + (d + c) \]
	\[ a + f = b + e, \] so $(a,b) \sim (e,f)$, so the relation $\sim$ is transitive.
\end{proof}
